...
实现的过程要有技术难度.
求新求变.
不能是纯开发的东西. 系统的实现上边没有什么创新性.

投CCKS. 构建了大规模课程知识图谱. 里边的概念和开放百科进行了关联. 数据获取后的处理, 建立联系等. 
从头到尾梳理一遍, 重写. 
课程图谱
概念识别 进行关联
路径规划 可以涉及一些算法. 
...
1.讲构建的必要性（需求）
2.论文中开发类的东西少写（技术方法少写），从算法/模型入手（课程的知识图谱，把课程和wikipedia的概念联系）
数据获取->定义本体->知识获取->图谱融合....

1.构建课程知识图谱（给学习者）提供知识服务
2.识别课程图谱里面的概念，跟dbpedia关联
3.学习路径的规划【想点算法】

1.先查一下课程图谱，列个提纲，梳理一下现在的工作。
2.有系统之后可以申请软件著作权。
